...
const Header = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const scrollThreshold = window.innerHeight * 0.1; // 30% of viewport height
  ...
  return (
    <nav className={`${styles.nav} ${isScrolled ? styles.scrolled : ''}`}>
      <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl padding-global padding-global-top">
        {/* Logo */}
        <a href="/" className={`${styles.logo} flex items-center ps-5`}>
          <img
            decoding="async"
            width="100"
            height="286"
        ...
        {/* Navigation Menu */}
        <div
          className={`${isMobileMenuOpen ? 'block' : 'hidden'} w-full lg:flex lg:w-auto lg:order-1 ${styles.navMenu}`}
          id="mobile-menu"
        >
        ...
        {/* Right Buttons */}
        <div className={`${styles.rightButtons} flex items-center lg:order-2`}>
          <button
            className={`${styles.getStartedBtn} font-medium rounded-lg px-4 lg:px-5 py-2 lg:py-2.5 mr-2 flex items-center gap-2`}
          >
            <svg
